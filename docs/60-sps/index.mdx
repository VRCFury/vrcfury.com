...
full backward-compatible replacement for DPS (Raliv), and TPS (Poiyomi Pro).
...
* If you are upgrading an existing legacy plug:
  * If the plug was DPS, change the shader for the plug's material from `Raliv/Penetrator` to `Standard` (or whatever other shader you want!)
  * If the plug was TPS, disable the `Penetrator` checkbox on the Poiyomi material
  * Remove any DPS `Tip` lights, which are commonly found in a child object named "Physics"
  * Delete any existing `SPS Plug` components. These can often be found on the same object as the mesh renderer if the "VRCFury Haptic Upgrader" was previously run on the avatar
...
> **Already have old DPS or TPS on your avatar?** Upgrade them following [these instructions](/sps/upgrade).
...
ScreenFX which depend on depth, such as edge outlines, edge glow, etc, may not follow SPS plug deformation.
You may see a "ghost" outline during this situation.
There is no solution to this, either disable the effect in the world's menu if possible, or use a different world.

### Realtime Unity Shadows

Realtime Unity shadows do not follow SPS plug deformation.
You may see "ghost" shadows inside of the mesh during this situation.
Either disable "Receive Shadows" on the mesh in the editor (if your avatar's look would be compatible), or choose a different
world that does not use realtime shadows (most do not). VRC Light Volumes still work properly.

### Triangles Drawn on Screen

In some extremely rare circumstates, you may see triangles visible on your screen when SPS is enabled (even if not deforming).
This can happen in worlds which use a solid color in place of a skybox, or which use camera re-rendering tricks.
...
There is no solution to this issue. Disable SPS when in these areas, or select a different world.

> Note: While this issue was technically fixable in the original design of SPS, we chose not to, as it would have added
a performance penalty everywhere, to only fix these few worlds.

### Backward Compatibility breaks when too many lights are nearby

DPS, TPS, and SPS v1 used lights to help with deformation. Unity imposes strict light limits, so when too many are nearby,
some of them are discarded. If you encounter this, ensure both parties are upgraded to SPS2, where light limits no longer apply.
