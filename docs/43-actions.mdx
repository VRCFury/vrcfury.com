...
# Actions
Many VRCFury components, such as Toggles, Gestures, SPS, Apply During Upload and more, can perform various types of "actions" when they are activated.
This page described the different types of actions that are available to use.
...
## Change SPS Tag
Animates SPS2 tags on an SPS Plug or SPS Socket.

This can be used to:

* Set a plug include tag
* Set a plug exclude tag
* Toggle a plug's `Global` SPS2 tag on or off
* Set a socket tag

When targeting a plug, the action can also control whether the tag applies to `Self`, `Others`, or both.
When targeting a socket, it edits one of that socket's 2 custom SPS2 tag slots.
