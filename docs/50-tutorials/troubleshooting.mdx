...
* Check any of your VRCFury component's headers for a `W` next to the version number. This indicates that you clicked `Skip and stop asking` when VRCFury detected a (likely) broken WD mix and asked whether or not it should try to fix it for you automatically. 
* If this is the case, find the [VRCFury | Fix Write Defaults](/other#fix-write-defaults) component under your avatar's root, and change it from `Disabled` to `Auto`
