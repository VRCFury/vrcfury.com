...
### Plug seems to "snap" toward socket while testing in editor

If you are using Av3Emulator, try Gesture Manager instead. Av3Emulator enables an option which improperly
adjusts the bounding box of the plug mesh, making it smaller than it should be which prevents it from "seeing"
the socket from far away.
